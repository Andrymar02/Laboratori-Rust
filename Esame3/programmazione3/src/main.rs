...
struct SharedState {
    data: Mutex<HashMap<String, Bucket>>,
}
...
impl ConcurrentCache for ConcurrentCacheImpl {
    fn new(d: Duration) -> Self {
        ...
        let worker = {
            // Cloniamo gli Arc: il thread avrà le sue copie dei puntatori
            // condivisi, mentre `shared`, `stop`, `cvar` restano disponibili
            // qui sotto per costruire `Self`.
            ...
            thread::spawn(move || {
                loop {
                    // Dormi fino allo scadere di `d`, oppure finché qualcuno
                    // (Drop) non notifica la condvar per svegliarci prima.
                    let guard = stop.lock().unwrap();
                    let (guard, _timeout_result) = cvar.wait_timeout(guard, d).unwrap();
                    let should_stop = *guard;
                    drop(guard); // rilasciamo il lock su `stop` prima di lavorare sulla mappa
                    ...
                    data.retain(|_, bucket| bucket.expires_at > now);
                }
            })
        };
    ...
    fn set(&self, key: &str, value: &str) {
        ...
        let mut data = self.shared.data.lock().unwrap();
        data.insert(key.to_string(), bucket);
    }
}
...
impl Drop for ConcurrentCacheImpl {
    fn drop(&mut self) {
        ...
        // Svegliamo subito il thread di background, invece di lasciarlo
        // dormire fino al prossimo timeout naturale.
...
fn main() {
    let cache = ConcurrentCacheImpl::new(Duration::from_secs(5));
